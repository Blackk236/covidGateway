...
export class PersonneService {
  public resourceUrl = this.applicationConfigService.getEndpointFor('api/personnes', 'covidrest');
  ...
  protected convertDateFromClient(personne: IPersonne): IPersonne {
    return Object.assign({}, personne, {
      date: personne.date?.isValid() ? personne.date.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((personne: IPersonne) => {
        personne.date = personne.date ? dayjs(personne.date) : undefined;
      });
    }
    return res;
  }
